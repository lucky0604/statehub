# OpenCode → StateHub MCP Setup

Connect [OpenCode](https://github.com/sst/opencode) to a StateHub workspace so
...
or your project's `.opencode.json`):
...
```json
{
  "mcp": {
    "statehub": {
      ...
      "headers": {
        "Authorization": "Bearer sth_REPLACE_WITH_YOUR_TOKEN"
      }
    }
  }
}
...
Inside the OpenCode session, ask the agent:
